...
router.post('/', authenticateToken, authorizeRoles('fleet_manager', 'safety_officer'), async (req, res) => {
  ...
  const parsedExpiry = new Date(license_expiry);
  if (isNaN(parsedExpiry.getTime())) {
    return res.status(400).json({ error: 'Invalid license expiry date format' });
  }

  const parsedScore = parseFloat(safety_score);
...
router.put('/:id', authenticateToken, authorizeRoles('fleet_manager', 'safety_officer'), async (req, res) => {
  const driverId = parseInt(req.params.id);
  if (isNaN(driverId)) {
    return res.status(400).json({ error: 'Invalid driver ID' });
  }
  ...
  try {
    ...
    if (license_expiry !== undefined) {
      ...
      updateData.license_expiry = parsedExpiry;
    }
    ...
    if (safety_score !== undefined) {
      ...
      updateData.safety_score = parsedScore;
    }
...
router.delete('/:id', authenticateToken, authorizeRoles('fleet_manager', 'safety_officer'), async (req, res) => {
  const driverId = parseInt(req.params.id);
  if (isNaN(driverId)) {
    return res.status(400).json({ error: 'Invalid driver ID' });
  }
  ...
  try {
    ...
    const activeTrips = await prisma.trips.findFirst({
      where: {
        driver_id: driverId,
        status: 'Dispatched',
      },
